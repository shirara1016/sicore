...
def _truncated_cdf_from_cdf(
    ...
    Args:
        cdf (callable[[float], float]): The CDF function of the distribution to be truncated.
        z (float): The value at which to compute the CDF.
        ...
        init_dps (int, optional): Initial dps value for mpmath when `dps` is set to "auto".
            Defaults to 30.
        max_dps (int, optional): Maximum dps value for mpmath when `dps` is set to "auto".
            Defaults to 5000.
    ...
    if z not in intervals:
        raise ValueError(f"The value {z} is not in the truncated intervals.")
    ...
    if dps == "auto":
        ...
        if len(finite_edge_points) != 0:
            ...
            literal = mp.nstr(
                cdf(extreme), init_dps - 15, min_fixed=-np.inf, max_fixed=np.inf
            )
            if literal == "1.0":
    ...
    if absolute:
        mask_intervals = RealSubset([[-np.abs(z), np.abs(z)]])
    else:
        mask_intervals = RealSubset([[-np.inf, z]])
